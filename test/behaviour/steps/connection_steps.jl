...
@step "connection opens with a wrong port; fails"
function (ctx)
    expect_throws() do
...
@step r"^connection opens with a wrong host; fails with a message containing: \"(.+)\"$"
function (ctx, msg)
...
@step r"^connection create database: (.+)$"
function (ctx, name)
    create_database(CTX.driver, strip(name))
end

@step r"^connection create database: (.+); fails$"
function (ctx, name)
    expect_throws() do
        create_database(CTX.driver, strip(name))
    end
...
@step r"^connection create database: (.+); fails with a message containing: \"(.+)\"$"
function (ctx, name, msg)
    expect_throws_msg(msg) do
        create_database(CTX.driver, strip(name))
    end
end
...
@step "connection create database with empty name; fails"
function (ctx)
    expect_throws() do
...
@step r"^connection create database with empty name; fails with a message containing: \"(.+)\"$"
function (ctx, msg)
    expect_throws_msg(msg) do
        create_database(CTX.driver, "")
    end
...
@step r"^connection delete database: (.+)$"
function (ctx, name)
    delete_database(CTX.driver, strip(name))
end

@step r"^connection delete database: (.+); fails$"
function (ctx, name)
    expect_throws() do
        delete_database(CTX.driver, strip(name))
    end
...
@step r"^connection delete database: (.+); fails with a message containing: \"(.+)\"$"
function (ctx, name, msg)
    expect_throws_msg(msg) do
        delete_database(CTX.driver, strip(name))
    end
...
# ─── Background tasks ─────────────────────────────────────────────────────────
@step r"^in background, connection create database: (.+)$"
function (ctx, name)
...
@step r"^in background, connection delete database: (.+)$"
function (ctx, name)
...
@step r"^in background, connection open schema transaction for database: (.+)$"
function (ctx, name)
    drv = CTX.driver
    t = @async begin
...
@step r"^connection open (read|write|schema) transaction for database: (.+)$"
function (ctx, type_name, db_name)
    CTX.transaction = _open_tx(strip(db_name), _tx_type_from_name(type_name))
end

@step r"^connection open (read|write|schema) transaction for database: (.+); fails$"
function (ctx, type_name, db_name)
    expect_throws() do
        CTX.transaction = _open_tx(strip(db_name), _tx_type_from_name(type_name))
    end
...
@step r"^connection open (read|write|schema) transaction for database: (.+); fails with a message containing: \"(.+)\"$"
function (ctx, type_name, db_name, msg)
    expect_throws_msg(msg) do
        CTX.transaction = _open_tx(strip(db_name), _tx_type_from_name(type_name))
    end
    CTX.transaction = nothing
...
@step r"^connection open transactions for database: (.+), of type:$"
function (ctx, db_name, datatable)
...
@step r"^connection open transactions in parallel for database: (.+), of type:$"
function (ctx, db_name, datatable)
...
@step r"^transaction is open: (true|false)$"
function (ctx, expected)
    if _is_true(expected)
...
@step "transactions in parallel are open: true"
function (ctx)
...
@step r"^transaction commits; fails with a message containing: \"(.+)\"$"
function (ctx, msg)
    expect_throws_msg(msg) do
        commit(CTX.transaction)
    end
...
@step "transaction rollbacks"
function (ctx)
    rollback(CTX.transaction)
end

@step "transaction rollbacks; fails"
function (ctx)
    expect_throws() do
        rollback(CTX.transaction)
    end
...
@step r"^transaction rollbacks; fails with a message containing: \"(.+)\"$"
function (ctx, msg)
    expect_throws_msg(msg) do
        rollback(CTX.transaction)
    end
...
# ─── Transaction options ───────────────────────────────────────────────────────
@step r"^set transaction option transaction_timeout_millis to: (\d+)$"
function (ctx, v)
    CTX.tx_options_timeout_ms = parse(Int64, v)
end

@step r"^set transaction option schema_lock_acquire_timeout_millis to: (\d+)$"
function (ctx, v)
    CTX.tx_options_schema_lock_ms = parse(Int64, v)
end

# ─── Schema retrieval ─────────────────────────────────────────────────────────
@step r"^connection get database\((\w[\w-]*)\) has schema:$"
function (ctx, db_name, docstring)
...
@step r"^connection get database\((\w[\w-]*)\) has type schema:$"
function (ctx, db_name, docstring)
    db   = get_database(CTX.driver, db_name)
